...
%% CONFIGS
% configs.path.base='\\AMPLPC29\He BEC Archive\EXPERIMENT-DATA\BELL_MF\20180122_bell-mf-global\bell_global\201805\exp4_bell_t_evo\0.95ms';
% configs.path.data=fullfile(configs.path.base,'data');

configs.path.base='C:\Users\HE BEC\bell_data\exp4_bell_t_evo\0.95ms\data';
...
configs.misc.param=0.95;     % DELAY (ms)
...
configs.mf(1).p_bec0=[1.6218,  -3.1e-3,    -1.5e-3;
                   1.6731,  -3.9e-3,    -1.4e-3];
configs.mf(1).r_bec0=8e-3;

configs.mf(2).mf=0;
configs.mf(2).window={vz*[0.4118,0.4293],35e-3*[-1,1],35e-3*[-1,1]};
configs.mf(2).p_bec0=[1.6868,  0.4e-3,    2.9e-3;
                    1.7407,  0.6e-3,    3.7e-3];
...
configs.post.Dk{2}=[0.015,0.005,0];