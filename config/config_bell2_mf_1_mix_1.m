...
%% FILES
configs.files.path='\\AMPLPC29\Users\TDC_user\ProgramFiles\my_read_tdc_gui_v1.0.1\dld_output\bell_v2\mix\mix1_3\d';
...
configs.load.id=1:487;         % file id numbers to use for analysis
...
configs.load.window{1}=[0.38,0.425];      % T [s]
...
configs.bec{1}.pos{1}=[vz*0.405,-3.4e-3,4.5e-3];   % approx condensate locations (z,x,y)
configs.bec{1}.Rmax{1}=12e-3;      % max condensate sph radius
configs.bec{1}.dR_tail{1}=1;     % BEC tail radial frac diff
configs.bec{1}.pos{2}=[vz*0.418,-2.9e-3,4.6e-3];
...
configs.bec{2}.pos{1}=[vz*0.387,-3.7e-3,3.3e-3];   % approx condensate locations (z,x,y)
configs.bec{2}.Rmax{1}=12e-3;      % max condensate sph radius
configs.bec{2}.dR_tail{1}=1;     % BEC tail radial frac diff
configs.bec{2}.pos{2}=[vz*0.399,-3.0e-3,3.7e-3];
...
configs.bell_lo.ampRaman=0.4;     % Raman amplitude (mixing), K_R_mix